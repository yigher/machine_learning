...
def cycle_gan(img_size=256):
    ...
    X_real = tf.placeholder(
        name='X', shape=[None, img_size, img_size, 3], dtype=tf.float32)
    Y_real = tf.placeholder(
        name='Y', shape=[None, img_size, img_size, 3], dtype=tf.float32)
...
def batch_generator_fixed_dataset(imgs1, imgs2):
    """Summary
    ...
    n_imgs = min(len(imgs1), len(imgs2))
    rand_idxs1 = np.random.permutation(np.arange(len(imgs1)))[:n_imgs]
    for idx1 in rand_idxs1:
        yield imgs1[[idx1]], imgs2[[idx1]]
...
def train(
    ...
    tf_param_str = 'img_size=' + str(img_size) +\
        ',learning_rate=' + str(learning_rate) +\
        ',n_epochs=' + str(n_epochs)

    log_dir = os.path.join(log_dir, time_stamp, tf_param_str)
    ckpt_path = os.path.join(ckpt_path, time_stamp)
    model_img_dir = os.path.join(model_img_dir, time_stamp)
    create_dir(log_dir)
    create_dir(ckpt_path)
    create_dir(model_img_dir)
    ...
    if ds_X.ndim == 3:
        ...
        if shuffle:
            ...
            batch_generator = batch_generator_fixed_dataset
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        test_xs = ds_X[:test_img_number]
        test_ys = ds_Y[:test_img_number]
        utils.montage(test_xs, os.path.join(model_img_dir, 'test_xs.png'))
        utils.montage(test_ys, os.path.join(model_img_dir, 'test_ys.png'))
            ...
            while not coord.should_stop() and epoch_i < n_epochs:
                for X, Y in batch_generator(ds_X, ds_Y):
                    ...
                    if it_i % update_summary_iter == 0:
                        ...
                        X_fake_recon, Y_fake_reon, X_cycle_recon, Y_cycle_recon = sess.run(
                            [net['X_fake'], net['Y_fake'], net['X_cycle'], net['Y_cycle']],
                            feed_dict={
                                net['X_real']: test_xs,
                                net['Y_real']: test_ys})
                        utils.montage(
                            X_fake_recon,
                            os.path.join(model_img_dir, 'reconXFake_%08d.png' % it_i))
                        utils.montage(
                            Y_fake_reon,
                            os.path.join(model_img_dir, 'reconYFake_%08d.png' % it_i))
                        utils.montage(
                            X_cycle_recon,
                            os.path.join(model_img_dir, 'reconXCycle_%08d.png' % it_i))
                        utils.montage(
                            Y_cycle_recon,
                            os.path.join(model_img_dir, 'reconYCycle_%08d.png' % it_i))

                    it_i += 1
                    sys.stdout.flush()
                epoch_i += 1
                # Save
                if epoch_i % save_epoch == 0:
                    saver.save(
                        sess,
