...
def VAE(input_shape=[None, 784],
    ...
    for layer_i, n_output in enumerate(n_filters[1:]):
        with tf.variable_scope('decoder/{}'.format(layer_i)):
            ...
            # print("shape: ", shape)
            # print("filter_size: ", filter_sizes[layer_i])
            if convolutional:
...
def train_vae(files,
    ...
    model_img_dir = "data/model_imgs/vae" + time.strftime('%Y%m%d_%H%M%S', time.gmtime())
    if not os.path.exists(model_img_dir):
        os.mkdir(model_img_dir)
    ...
    try:
        while not coord.should_stop() and epoch_i < n_epochs:
            ...
            train_cost = sess.run([ae['cost'], optimizer], feed_dict={
                ...
                ae['corrupt_prob']: [corrupt_prob]})[0]
