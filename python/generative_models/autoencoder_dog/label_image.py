...
parser.add_argument(
    '--image_dir', required=False, type=str, help='Absolute path image directory.')
parser.add_argument(
...
def run_graph(image_id, image_data, labels, input_layer_name, output_layer_name,
              num_top_predictions):
  with tf.Session() as sess:
    ...
    return image_id + "," +",".join(map(str, predictions))
...
def main(argv):
  ...
  if FLAGS.image is not None:
    ...
    # get Id
    image_id = get_id_from_file(FLAGS.image)
    
    predictions = run_graph(image_id, image_data, labels, FLAGS.input_layer, FLAGS.output_layer,
              FLAGS.num_top_predictions)
    print("id, "+",".join(labels))
    print(predictions)
  elif FLAGS.image_dir is not None:
    image_files = os.listdir(FLAGS.image_dir)
    image_files = [os.path.join(FLAGS.image_dir, f) for f in image_files]
    ...
    with open(FLAGS.output_prediction_file, 'w') as f:
      ...
      for i, image_file in enumerate(image_files):
        ...
        # get Id
        image_id = get_id_from_file(image_file)
        print("Processing file number ", i, ": ", image_file)
        predictions = run_graph(image_id, image_data, labels, FLAGS.input_layer, FLAGS.output_layer,
                  FLAGS.num_top_predictions)
        f.write(predictions+"\n")
    f.close()
